...
func (au *ActionUpdate) AddPermissionIDs(ids ...int) *ActionUpdate {
	au.mutation.AddPermissionIDs(ids...)
	return au
}

// AddPermission adds the "permission" edges to the Permission entity.
func (au *ActionUpdate) AddPermission(p ...*Permission) *ActionUpdate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (au *ActionUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	_spec := sqlgraph.NewUpdateSpec(action.Table, action.Columns, sqlgraph.NewFieldSpec(action.FieldID, field.TypeUint64))
	...
	if nodes := au.mutation.RemovedPermissionIDs(); len(nodes) > 0 && !au.mutation.PermissionCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   action.PermissionTable,
	...
	if nodes := au.mutation.PermissionIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   action.PermissionTable,
...
// AddPermissionIDs adds the "permission" edge to the Permission entity by IDs.
func (auo *ActionUpdateOne) AddPermissionIDs(ids ...int) *ActionUpdateOne {
	auo.mutation.AddPermissionIDs(ids...)
	return auo
}

// AddPermission adds the "permission" edges to the Permission entity.
func (auo *ActionUpdateOne) AddPermission(p ...*Permission) *ActionUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
// ClearPermission clears all "permission" edges to the Permission entity.
func (auo *ActionUpdateOne) ClearPermission() *ActionUpdateOne {
	auo.mutation.ClearPermission()
	return auo
}

// RemovePermissionIDs removes the "permission" edge to Permission entities by IDs.
func (auo *ActionUpdateOne) RemovePermissionIDs(ids ...int) *ActionUpdateOne {
	auo.mutation.RemovePermissionIDs(ids...)
	return auo
}

// RemovePermission removes "permission" edges to Permission entities.
func (auo *ActionUpdateOne) RemovePermission(p ...*Permission) *ActionUpdateOne {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (auo *ActionUpdateOne) sqlSave(ctx context.Context) (_node *Action, err error) {
	...
	if nodes := auo.mutation.RemovedPermissionIDs(); len(nodes) > 0 && !auo.mutation.PermissionCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   action.PermissionTable,
	...
	if nodes := auo.mutation.PermissionIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   action.PermissionTable,
