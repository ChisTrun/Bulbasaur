...
func (aq *ActionQuery) QueryPermission() *PermissionQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, action.PermissionTable, action.PermissionColumn),
		)
...
func (aq *ActionQuery) IDs(ctx context.Context) (ids []uint64, err error) {
...
func (aq *ActionQuery) Clone() *ActionQuery {
	...
	return &ActionQuery{
		...
		withPermission: aq.withPermission.Clone(),
...
func (aq *ActionQuery) WithPermission(opts ...func(*PermissionQuery)) *ActionQuery {
	query := (&PermissionClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withPermission = query
	return aq
}
...
func (aq *ActionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Action, error) {
	...
	if query := aq.withPermission; query != nil {
		if err := aq.loadPermission(ctx, query, nodes,
			func(n *Action) { n.Edges.Permission = []*Permission{} },
			func(n *Action, e *Permission) { n.Edges.Permission = append(n.Edges.Permission, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *ActionQuery) loadPermission(ctx context.Context, query *PermissionQuery, nodes []*Action, init func(*Action), assign func(*Action, *Permission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*Action)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(permission.FieldActionID)
	}
	query.Where(predicate.Permission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(action.PermissionColumn), fks...))
	}))
...
func (aq *ActionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(action.Table, action.Columns, sqlgraph.NewFieldSpec(action.FieldID, field.TypeUint64))
	_spec.From = aq.sql
...
func (aq *ActionQuery) ForUpdate(opts ...sql.LockOption) *ActionQuery {
...
func (aq *ActionQuery) ForShare(opts ...sql.LockOption) *ActionQuery {
	if aq.driver.Dialect() == dialect.Postgres {
		aq.Unique(false)
	}
	aq.modifiers = append(aq.modifiers, func(s *sql.Selector) {
