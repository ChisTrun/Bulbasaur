...
// QueryGoogle chains the current query on the "google" edge.
func (uq *UserQuery) QueryGoogle() *GoogleQuery {
	query := (&GoogleClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithGoogle tells the query-builder to eager-load the nodes that are connected to
// the "google" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithGoogle(opts ...func(*GoogleQuery)) *UserQuery {
	query := (&GoogleClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) loadGoogle(ctx context.Context, query *GoogleQuery, nodes []*User, init func(*User), assign func(*User, *Google)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(google.FieldUserID)
	}
	query.Where(predicate.Google(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.GoogleColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
